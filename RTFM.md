...
### Adding Users
To add or update a user securely, use the CLI built-in or the `adduser.sh` wrapper (both use the same implementation):
```bash
aethercli --adduser admin
# Or, pointing at a specific users.json:
./adduser.sh /etc/aethercli/users.json admin
```
